...
function generatePDF($demande, $etudiant, $notes) {
    ...
    $logoPath = '../ensate.png'; 
    $pdf->Image($logoPath, 10, 10, 60); // Ajustez les coordonnées et la taille du logo 
    // Définir une police pour le titre
    $pdf->SetFont('Arial', 'B', 25);
    // Positionner le titre au milieu de la page 
    $pdf->SetY(50); 
    $pdf->Cell(0, 10, 'Releve de notes', 0, 1, 'C'); // Centrer le titre
    // Ajouter un saut de ligne après le titre
    $pdf->Ln(10);
    $pdf->SetFont('Times', '', 13);   
    $pdf->Cell(0, 10, 'Etudiant : ' . utf8_decode($etudiant['fullName']), 0, 1);
    $pdf->Cell(0, 10, 'Filiere : ' . utf8_decode($etudiant['niveau']), 0, 1);
    // Vérifier si la demande existe
    if ($demande) {
        $pdf->Cell(0, 10, 'semestre : S' . utf8_decode($notes[0]['semestre']), 0, 1);
        // Ajouter les notes
        foreach ($notes as $note) {
            $pdf->Cell(0, 10, utf8_decode($note['module']) . ': ' . utf8_decode($note['valeur']), 0, 1);
        }
    }
    $pdf->Cell(0, 6, 'Monsieur Kamal REKLAOUI', 0, 1, 'R');
    $filename = 'releve_note_' . utf8_decode($etudiant['fullName']) . '.pdf';
    $filepath = '../Document/releve de note' . $filename;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['accepter'])) {
    ...
    // Vérifier si la demande a renvoyé des résultats
    if ($demande) {
        ...
        if ($resultStudent) {
            ...
            $resultnote = $db->query("SELECT * FROM notes WHERE id_etudiant = $idstudent");
            $notes = $resultnote->fetchAll(PDO::FETCH_ASSOC);
            // Génération du PDF
            $pdfData = generatePDF($demande, $etudiant, $notes);
            ...
            $stmt->execute();
            $sujet = 'Releve de notes ';
            $message = 'Vous trouvez ci joint le releve de notes que vous avez demandé' ;
...
if (isset($_POST['confirmer_refus'])) {
    ...
    if ($demande) {
        ...
        $stmt = $db->prepare("INSERT INTO demande_refus (motifrefus,idstudent,type_demande, date_demande) VALUES (?,?,'releve de notes', '$date_demande')");
        $stmt->bindValue(1, $motifRefus);
        $stmt->bindValue(2, $idStudent);
        $stmt->execute();
    }
    $sujet='Refus de la demande de releve de notes ';
...
$result = $db->query("SELECT * FROM demande WHERE typedemande = 'Relevé de notes' ORDER BY id_demande DESC ");
...
<head>
    ...
    <title>Liste des demandes de releve de notes</title>
</head>
<body>
    <div class="container">
    ...
    <div class="content" >    
    <h1 class="title3">Liste des demandes de releve de notes </h1>
