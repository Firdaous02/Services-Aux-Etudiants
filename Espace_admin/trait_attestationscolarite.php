...
function generatePDF($demande, $etudiant) {
    ...
    $pdf->Cell(0, 10, 'Attestation de scolarite', 0, 1, 'C'); // Centrer le titre
    ...
    $filename = 'attestation_scolarite_' . $etudiant['fullName'] . '.pdf';
    $filepath = '../Document/attestation scolarite' . $filename;
...
$result = $db->query("SELECT * FROM demande WHERE typedemande = 'attestation de scolarité'");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['accepter'])) {
        ...
        if ($demande) {
            ...
            if ($resultStudent) {
                ...
                $typeDemande = 'Attestation de scolarite';
                $stmt->bindValue(2, $typeDemande);
                $stmt->bindValue(3, $pdfData['filename']);
                $stmt->bindValue(4, $pdfData['content'], PDO::PARAM_LOB);
                $stmt->execute();
                $sujet = 'Attestation de scolarite ';
                $message = 'Vous trouvez ici ci-joint l\'attestation de scolarite que vous avez demandé' ;
    ...
    if (isset($_POST['confirmer_refus'])) {
        ...
        if ($demande) {
            ...
            $stmt = $db->prepare("INSERT INTO demande_refus (motifrefus,idstudent,type_demande, date_demande) VALUES (?,?,'attestation de scolarite', '$date_demande')");
...
$result = $db->query("SELECT * FROM demande WHERE typedemande = 'attestation de scolarite' ORDER BY id_demande DESC");
$demandes = $result->fetchAll(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html>
    <head>
        ...
        <title>Liste des demandes d'attestation de scolarite</title>
    </head>
    <body>
    <div class="container">
        ...
        <div class="content" > 
            <h1 class="title3">Liste des demandes d'attestation de scolarité non traitées </h1>
